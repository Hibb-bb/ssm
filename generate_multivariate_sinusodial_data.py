"""
Generate synthetic multivariate sinusoidal datasets with controlled temporal
irregularity for the ssm_dk IMTS benchmark (v2, phase 2 — dense synchronized).

This is a verbatim port of the reference generator at
...
with two intentional deviations:
  1. HISTORY = 8.0 (reference uses 7.0); matches the ssm_dk v1 convention so
     Phase 2 results compare cleanly with the prior v1 runs.
  2. Default output_root points into ssm_dk/data_correct/ instead of the
     reference's tpatchgnn_data/.

Four irregularity levels (controlled by fraction of identical Δt):
  ...
  - regular:    100% identical gaps

Signals (shared timestamps per sample; sparse flat layout):
  - Variate 1: x1(t) = A1 * sin(2π * f1 * t + φ1) + ε1
  - Variate 2: x2(t) = A2 * sin(2π * f2 * t + φ2) + ε2
  - Variate 3: x3(t) = w * x1(t) + (1 - w) * x2(t)

  f ∈ [0.5, 3.0], A ∈ [0.5, 2.0], φ ∈ [0, 2π), ε ~ N(0, 0.05)
  w uniform in (W_MIX_LO, W_MIX_HI) to avoid degenerate weights.

The same underlying signal parameters and noise draws are used across
irregularity levels so the only difference is temporal sampling.

Usage:
    python generate_multivariate_sinusodial_data.py [--output_root PATH] [--n_train 1000]
"""
...
def build_dataset(
    ...
    for i in range(n_samples):
        ...
        v1 = evaluate_signal(timestamps, freqs1[i], amps1[i], phases1[i], signal_rng)
        v2 = evaluate_signal(timestamps, freqs2[i], amps2[i], phases2[i], signal_rng)
        w = float(mix_w[i])
        v3 = (w * v1 + (1.0 - w) * v2).astype(np.float32)
        ...
        dt_list = delta_t.tolist()

        target = flat_vals.tolist()
        stamp = ts_list * N_VARS
        past = dt_list * N_VARS
...
def main():
    ...
    if args.output_root is None:
        output_root = Path(__file__).resolve().parent / "data_correct"
    else:
    ...
    for level_name, frac_regular in IRREGULARITY_LEVELS.items():
        print(f"\n{'=' * 60}")
        print(f"Generating: multisin_{level_name}")
