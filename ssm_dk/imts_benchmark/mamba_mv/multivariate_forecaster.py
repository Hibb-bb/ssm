...
class MultivariateMambaForecaster(pl.LightningModule):
    ...
    def _mask_prediction_inputs(self, values, timestamps, valid_mask):
        """Zero-out values in the prediction region. Identical semantics to
        the existing univariate MambaForecaster._mask_prediction_inputs."""
        pred_region = timestamps >= self.history
        masked = values.clone()
    ...
    def forward(self, batch):
        ...
        masked_values, _ = self._mask_prediction_inputs(values, timestamps, valid_mask)
    ...
    def test_step(self, batch, batch_idx):
        ...
        for i in range(B):
            ...
            # Per-variate MSE (useful for in-gap vs out-of-gap analysis: the
            # gapped variate has fewer observations than its peers).
            per_var_mse = {}
