...
- `aggregate_hpo_winners_physionet.py` — picks val/MSE-best `(lr, eff_bs)` per `dt_mode` from the 27-cell sweep and writes the JSON consumed by the confirm sbatch.
...
### SLURM submission (Phase 5 real on Delta)

The two submit scripts encode the canonical dependency chain:
- `bash imts_benchmark/scripts/submit_physionet_delta.sh` — smoke (1 job) → S5 (5) + RoMAE (5) + Mamba-MV (15), all `afterok:smoke`.
- `SMOKE_ID=<jobid> bash imts_benchmark/scripts/submit_hpo_physionet_delta.sh` — sweep (27) `afterok:smoke` → aggregator `afterany:sweep` → confirm (15) `afterok:aggregator`. The aggregator uses `afterany` deliberately so partial-failure sweeps still produce winners as long as each `dt_mode` has at least one usable cell.

Both support `DRY_RUN=1`. Match real-dataset sbatches by suffix: `_delta` = DeltaAI (aarch64/GH200), `_delta_x86` = Delta (x86_64/A100/A40); plain (no suffix) = Quest. Many `*_delta_x86.sbatch.bak` files are last-known-good before edits — keep them.
...
- **Vendored upstream paths are gitignored and read-only**: `imts_benchmark/*/_upstream/` (S5) and `_romae_repo/` (RoMAE clone). Don't edit; rebuild via the env scripts.
