...
irregularity for ablation study of RoPE + delta-t on MOIRAI.
...
Signals (shared timestamps per sample; sparse flat layout as in convert_tpatchgnn_data):
  - Variate 1: x1(t) = A1 * sin(2π * f1 * t + φ1) + ε1
  - Variate 2: x2(t) = A2 * sin(2π * f2 * t + φ2) + ε2
  - Variate 3: x3(t) = w * x1(t) + (1 - w) * x2(t)  (convex mix, same t as x1,x2)

  f ∈ [0.5, 3.0], A ∈ [0.5, 2.0], φ ∈ [0, 2π), ε ~ N(0, 0.05)
  w uniform in (W_MIX_LO, W_MIX_HI) to avoid degenerate weights.

The same underlying parameters and noise draws are used across irregularity
...
HISTORY = 7.0
NOISE_STD = 0.05
# Convex weights w in (lo, hi) so variate 3 is not trivially one source.
W_MIX_LO = 0.05
W_MIX_HI = 0.95
...
def generate_multivariate_signal_params(n_samples, rng):
    """Two independent sinusoids per sample plus convex mix weight w."""
...
def build_dataset(
    ...
    Sparse multivariate flat layout: target / timestamp / past_feat are
    concatenated per variate (same ts and delta_t repeated for each variate).
...
def compute_irregularity_stats(rows):
    """Compute CV of Δt and mode fraction for a set of samples (first variate only)."""
    all_cv = []
    all_mode_frac = []
    for row in rows:
        # Flat layout repeats delta_t per variate; use first block only.
