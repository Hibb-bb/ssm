"""Pick the val/mse-best (lr, eff_bs) per dt_mode from a 27-cell IMM Mamba-MV
HPO sweep and emit a winners JSON consumed by the IMM confirm sbatch.

Generalized version of aggregate_hpo_winners_physionet.py: takes the eff_bs grid
as a CLI flag so we can pass smaller grids for tiny datasets (CESNET / ILINet).

Sweep layout (mirrors run_mamba_mv_hpo_imm_delta_x86.sbatch):
  <hpo_log_dir>/<dt_mode>/lr-<lr>_ebs-<ebs>/seed1/checkpoints/best.ckpt

eff_bs -> (phys_bs, accum) translation matches the sbatch case statement.

Usage:
  python -m imts_benchmark.eval.aggregate_hpo_winners_imm \
      --hpo_log_dir output/log/imts_benchmark_v2_imm/hpo_mamba_mv_imm_fnspid \
      --eff_bs 64 128 256
"""
...
def main() -> int:
    ...
    p.add_argument("--eff_bs", type=int, nargs=3, default=[64, 128, 256],
                   help="eff_bs grid used in the sweep (3 values)")
    p.add_argument("--seed", type=int, default=1)
    args = p.parse_args()

    hpo_dir = Path(args.hpo_log_dir)
    if not hpo_dir.exists():
        print(f"[hpo_imm] ERROR: HPO dir not found: {hpo_dir}", file=sys.stderr)
        return 1

    for ebs in args.eff_bs:
        if ebs not in EBS_TO_PHYS:
            print(f"[hpo_imm] ERROR: unknown eff_bs={ebs}; supported: {sorted(EBS_TO_PHYS)}",
                  file=sys.stderr)
            return 1
    ...
    print(f"[hpo_imm] wrote {ranked_csv}")

    winners: dict = {}
    for dt in DT_MODES:
        sub = df[df["dt_mode"] == dt]
        if len(sub) == 0 or sub["min_val_mse"].min() == float("inf"):
            print(f"[hpo_imm] WARNING: no usable cells for dt_mode={dt}", file=sys.stderr)
            continue
        best = sub.iloc[0]
        winners[dt] = {
            "dt_mode": dt,
    ...
    print(f"[hpo_imm] wrote {winners_json}")
    ...
    if missing:
        print(f"\n[hpo_imm] WARNING: {len(missing)} cells missing/unreadable:",
              file=sys.stderr)
        for m in missing[:5]:
    ...
    if len(winners) < len(DT_MODES):
        print(f"[hpo_imm] ERROR: only {len(winners)}/{len(DT_MODES)} dt_modes have winners",
              file=sys.stderr)
        return 2
