...
class RoMAEForecaster(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Phase 4 gap info (empty for Phase 2/3).
        B = values.shape[0]
        ...
        for i in range(B):
            ...
            ts_flat = timestamps[i][real_mask].detach().cpu().numpy()
            ...
            # d varies per sample per gapped_variate_index.
            if (i < len(gap_starts_list) and gap_starts_list[i]
                ...
                vd_mask = (variates == d)
                if vd_mask.sum() > 0:
                    ts_vd = ts_flat[vd_mask]
                    y_t_vd = y_true[vd_mask]
                    y_p_vd = y_pred[vd_mask]
