...
For our forecasting task we frame it as MAE-style interpolation:
  * Build per-sample **sorted flat sequence of (variate, time, value) tuples** —
    one token per observation, preserved as separate tokens even if two variates
    happen to share a timestamp. (Earlier comments said "union"; we preserve
    duplicates with one-hot variate encoding, not a set-theoretic merge.)
  * Encoder input x: at each token, value goes in column var_id of a [2V] vector
    (other columns 0); mask channel is one-hot on var_id.
...
class ContiFormerForecaster(pl.LightningModule):
    ...
    def _build_inputs(self, batch):
        ...
        Returns:
            ...
            time_steps     [B, T_max]      sorted flat concatenation of
                            observation times (one entry per observation;
                            duplicates preserved across variates)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Phase 4 gap info (empty lists for Phase 2/3).
