"""Multivariate Mamba classifier (PyTorch Lightning).

Reuses the forecaster encoder verbatim (per-variate irregular SSM -> shared
grid -> L x (variable-axis attention + temporal Mamba)) and replaces the
forecasting query readout with a hierarchical availability-masked gated pool
classification head. Trained end-to-end with class-weighted cross-entropy.

Design contract:
    forward(batch) -> dict(z [B, D], logits [B, C])
    z is exposed for a future frozen-encoder linear probe track.
"""
...
class MultivariateMambaClassifier(pl.LightningModule):
    def __init__(
        ...
        # Class weights: registered as buffer; pass tensor of length n_classes
        # at construction or set later via `set_class_weights`.
    ...
    def forward(self, batch) -> dict:
        ...
        # Classification: entire window is observed evidence; no
        # pred-region masking (forecaster's _mask_prediction_inputs is dropped).
        h_pv = self.perv_ssm(values, deltat, valid_mask)          # [B,V,L,D_h]
