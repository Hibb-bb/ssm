#!/usr/bin/env bash
# Same as run_mamba_mv_tsking.sh, but MultivariateMambaSandwichForecaster:
#   n_perv_layer=2, n_fusion_blocks=2, n_tail_grid_mamba=2.
# Default: seeds 1..10 sequentially.
#
# Usage:
#   export WANDB_API_KEY="..."
#   bash run_mamba_mv_tsking_sandwich.sh
#
#   SEED_START=1 SEED_END=3 bash run_mamba_mv_tsking_sandwich.sh
#   SKIP_DATA_GEN=1 bash run_mamba_mv_tsking_sandwich.sh
#   USE_SRUN=1 bash run_mamba_mv_tsking_sandwich.sh
#   DRY_RUN=1 bash run_mamba_mv_tsking_sandwich.sh
...
OUTPUT_BASE="${OUTPUT_BASE:-${REPO_DIR}/output/mamba_mv_tsking_sandwich}"
...
echo "=========================================="
echo "arch:        sandwich (2 irregular + 2 fusion + 2 grid tail)"
...
for SEED in $(seq "${SEED_START}" "${SEED_END}"); do
  RUN_IDX=$((RUN_IDX + 1))
  TAG="${REGIME}_${DT_MODE}_seed${SEED}_sandwich222"
